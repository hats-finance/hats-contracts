...
contract("HatVaults", (accounts) => {
  ...
  it("custom bountySplit and max bounty", async () => {
    ...
    try {
      await hatVaults.setPendingMaxBounty(
        0,
        11000,
        { from: accounts[1] }
      );
    ...
    tx = await hatVaults.setPendingMaxBounty(
      0,
      10000,
      { from: accounts[1] }
    );
    ...
    await advanceToSaftyPeriod();
    await hatVaults.submitClaim(0, accounts[2], 10000, "description hash", {
      from: accounts[1] });
    try {
  ...
  it("approve + stake + exit", async () => {
    ...
    try {
      await hatVaults.submitClaim(0, utils.NULL_ADDRESS, 8000, "description hash", {
        from: accounts[1] });
  ...
  it("transferReward to fail if not enough reward tokens", async () => {
    await setup(accounts, "100000", (await web3.eth.getBlock("latest")).number,
        8000, undefined, undefined, undefined, undefined, undefined, 1);
    var staker = accounts[4];
    await stakingToken.approve(hatVaults.address, web3.utils.toWei("1"), {
      from: staker,
    });
