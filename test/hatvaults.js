...
contract("HatVaults", (accounts) => {
  ...
  async function safeEmergencyWithdraw(vault, staker) {
    let withdrawPeriod = (
      await hatVaultsRegistry.generalParameters()
    ).withdrawPeriod.toNumber();
    let safetyPeriod = (
    ...
    return await vault.emergencyWithdraw({ from: staker });
  }

  async function unSafeEmergencyWithdraw(vault, staker) {
    let currentTimeStamp = (await web3.eth.getBlock("latest")).timestamp;
    let withdrawPeriod = (
      await hatVaultsRegistry.generalParameters()
    ).withdrawPeriod.toNumber();
  ...
  it("withdrawn request ", async () => {
    ...
    try {
      await vault.emergencyWithdraw({ from: staker });
  ...
  it("multiple stakes from same account", async () => {
    ...
    await rewardController.getPastEvents('SafeTransferReward', {
        fromBlock: tx.blockNumber,
        toBlock: tx.blockNumber
    })
    .then(function(logs){
        assert.equal(logs[0].event, "SafeTransferReward");
        assert.equal(logs[0].args.amount.toString(), expectedReward.toString());
        assert.equal(logs[0].args.user, staker);
        assert.equal(logs[0].args.vault, vault.address);
  ...
  it("hat reward withdraw all balance if reward larger than balance", async () => {
    ...
    assert.equal(logs[0].event, "SafeTransferReward");
    assert.isTrue(
      parseInt(logs[0].args.amount.toString()) >=
    ...
    assert.equal(
      (await hatToken.balanceOf(staker)).toString(),
      logs[0].args.amount.add(balanceOfStakerBefore).toString()
    );
    ...
    await rewardController.updateVault(vault.address);
    balanceOfStakerBefore = await hatToken.balanceOf(staker);
    tx = await safeRedeem(vault, web3.utils.toWei("4"), staker);
    logs = await rewardController.getPastEvents('SafeTransferReward', {
        fromBlock: tx.blockNumber,
        toBlock: tx.blockNumber
    });
  ...
  it("emergency withdraw", async () => {
    ...
    assert.equal(await stakingToken.balanceOf(staker), 0);
    try {
      await unSafeEmergencyWithdraw(vault, staker);
  ...
  it("approve + stake + exit", async () => {
    ...
    let stakerAmount = await vault.balanceOf(staker);
    assert.equal(stakerAmount.toString(), web3.utils.toWei("1"));
    tx = await safeRedeem(vault, stakerAmount, staker);
    ...
    assert.equal(logs[0].event, "SafeTransferReward");
    let totalReward = logs[0].args.amount;
    ...
    tx = await safeRedeem(vault, stakerAmount, staker2);
    logs = await rewardController.getPastEvents('SafeTransferReward', {
        fromBlock: tx.blockNumber,
        toBlock: tx.blockNumber
    });
    assert.equal(logs[0].event, "SafeTransferReward");
    totalReward = totalReward.add(logs[0].args.amount);
    assert.equal(
      (await hatToken.balanceOf(rewardController.address)).toString(),
      new web3.utils.BN(
        web3.utils.toWei(rewardControllerExpectedHatsBalance.toString())
  ...
  it("emergencyWithdraw after approve and check reward", async () => {
    ...
    await safeEmergencyWithdraw(vault, staker);
    await vault.deposit(web3.utils.toWei("1"), staker, { from: staker });
    let tx = await safeRedeem(vault, web3.utils.toWei("1"), staker2);
    let logs = await rewardController.getPastEvents('SafeTransferReward', {
        fromBlock: tx.blockNumber,
        toBlock: tx.blockNumber
    });
  ...
  it("deposit mint withdraw redeem after approve claim", async () => {
    ...
    assert.equal(tx.logs[4].args.assets.toString(), web3.utils.toWei("0.2"));
  ...
  it("emergencyWithdraw after approve", async () => {
    ...
    tx = await safeEmergencyWithdraw(vault, staker2);
    assert.equal(tx.logs[3].event, "EmergencyWithdraw");
    assert.equal(tx.logs[3].args.assets.toString(), web3.utils.toWei("1"));
    assert.equal(tx.logs[3].args.shares.toString(), web3.utils.toWei("1"));
  ...
  it("approve + swapBurnSend", async () => {
    ...
    try {
      await vault.swapBurnSend(accounts[2], 0, router.address, payload);
    ...
    amountToSwapAndBurn = await vault.swapAndBurn();
    amountForHackersHatRewards = await vault.hackersHatReward(
      accounts[2]
    );
    ...
    let tx = await vault.swapBurnSend(
      accounts[2],
      0,
      router.address,
      payload
    ...
    assert.equal(tx.logs[6].event, "SwapAndBurn");
    var expectedHatBurned = new web3.utils.BN(web3.utils.toWei("0.8"))
      .mul(new web3.utils.BN("250"))
      .div(new web3.utils.BN(10000));
    assert.equal(
      tx.logs[6].args._amountBurned.toString(),
      expectedHatBurned.toString()
    );
    assert.equal(tx.logs[9].event, "SwapAndSend");
    var vestingTokenLock = await HATTokenLock.at(tx.logs[9].args._tokenLock);
    assert.equal(
    ...
    assert.equal(
      ...
      tx.logs[9].args._amountReceived.toString()
    );
  ...
  it("approve+ swapBurnSend with HAT vault", async () => {
    ...
    let tx = await newVault.swapBurnSend(
      accounts[2],
      0,
      router.address,
      payload
    ...
    var vestingTokenLock = await HATTokenLock.at(tx.logs[3].args._tokenLock);
    assert.equal(
      (await hatToken.balanceOf(vestingTokenLock.address)).toString(),
  ...
  it("swapAndBurn bounty check", async () => {
    ...
    assert.equal(tx.logs[5].event, "SwapAndBurn");
    assert.equal(
      tx.logs[5].args._amountSwapped.toString(),
    ...
    assert.equal(
      tx.logs[5].args._amountBurned.toString(),
      new web3.utils.BN(web3.utils.toWei("0.8"))
        .mul(
          new web3.utils.BN(
            (await vault.bountySplit()).swapAndBurn
    ...
    assert.equal(
      tx.logs[8].args._amountReceived.toString(),
      new web3.utils.BN(web3.utils.toWei("0.8"))
        .mul(
          new web3.utils.BN(
            (await vault.bountySplit()).hackerHatVested
    ...
    let afterBountyBalance = (
      await hatToken.balanceOf(tx.logs[8].args._tokenLock)
    ).toString();
    assert.equal(
      tx.logs[8].args._amountReceived.toString(),
      afterBountyBalance
  ...
  it("swapBurnSend", async () => {
    ...
    try {
      await vault.swapBurnSend(accounts[1], 0, accounts[1], payload, {
        from: accounts[0],
      });
    ...
    assert.equal(tx.logs[6].event, "SwapAndSend");
    assert.equal(tx.logs[6].args._amountReceived.toString(), "0");
    // Not real beneficiary should not get tokens
    let afterBountyBalance = (
      await hatToken.balanceOf(tx.logs[6].args._tokenLock)
    ).toString();
    assert.equal(
      tx.logs[6].args._tokenLock,
    ...
    assert.equal(tx.logs[5].event, "SwapAndBurn");
    assert.equal(tx.logs[5].args._amountBurned.toString(), "0");
    assert.equal(
      tx.logs[8].args._amountReceived.toString(),
      new web3.utils.BN(web3.utils.toWei("0.8"))
        .mul(
  ...
  it("swapBurnSend 2 vaults with same token", async () => {
    ...
    let vaults = [vault, newVault];
    for (i = 0; i < 2; i++) {
      let path = ethers.utils.solidityPack(
        ["address", "uint24", "address", "uint24", "address"],
        [stakingToken.address, 0, utils.NULL_ADDRESS, 0, hatToken.address]
      );
      let amountToSwapAndBurn = await vaults[i].swapAndBurn();
      let amountForHackersHatRewards = await vaults[i].hackersHatReward(
        accounts[1]
      );
      let amount = amountToSwapAndBurn
        .add(amountForHackersHatRewards)
        .add(await vaults[i].governanceHatReward());
      let payload = ISwapRouter.encodeFunctionData("exactInput", [
        [path, vaults[i].address, 0, amount.toString(), 0],
      ]);
      let tx = await vaults[i].swapBurnSend(
        accounts[1],
        0,
        router.address,
        payload,
      ...
      assert.equal(tx.logs[5 + i].event, "SwapAndBurn");
      assert.equal(
        tx.logs[5 + i].args._amountSwapped.toString(),
        new web3.utils.BN(web3.utils.toWei("0.8"))
          .mul(
            new web3.utils.BN(
              (await vaults[i].bountySplit()).swapAndBurn
            ).add(
              new web3.utils.BN(
                (await vaults[i].bountySplit()).governanceHat
              )
            )
          )
          .div(new web3.utils.BN("10000"))
      ...
      assert.equal(
        tx.logs[5 + i].args._amountBurned.toString(),
        new web3.utils.BN(web3.utils.toWei("0.8"))
          .mul(
            new web3.utils.BN(
              (await vaults[i].bountySplit()).swapAndBurn
      ...
      assert.equal(tx.logs[6 + i].event, "SwapAndSend");
      assert.equal(tx.logs[6 + i].args._amountReceived.toString(), "0");
      // Not real beneficiary should not get tokens
      let afterBountyBalance = (
        await hatToken.balanceOf(tx.logs[6 + i].args._tokenLock)
      ).toString();
      assert.equal(
        tx.logs[6 + i].args._tokenLock,
        "0x0000000000000000000000000000000000000000"
      );
      assert.equal(
        tx.logs[6 + i].args._amountReceived.toString(),
        afterBountyBalance
      );
    }
    ...
    for (i = 0; i < 2; i++) {
      ...
      assert.equal(
        ...
        new web3.utils.BN(web3.utils.toWei("0.8"))
          .mul(
            new web3.utils.BN(
              (await vaults[i].bountySplit()).hackerHatVested
            )
          )
          .div(new web3.utils.BN("10000"))
          .toString()
  ...
  it("no vesting", async () => {
    ...
    let tx = await vault.approveClaim(8000);
    assert.equal(tx.logs[2].event, "ApproveClaim");
    assert.equal(tx.logs[2].args._tokenLock, utils.NULL_ADDRESS);
    assert.equal(
      await stakingToken.balanceOf(vault.address),
  ...
  it("stop in the middle", async () => {
    ...
    assert.equal(await stakingToken.balanceOf(staker), web3.utils.toWei("2"));
    let userHatRewards = logs[0].args.amount;
    assert.equal(
    ...
    userHatRewards = userHatRewards.add(logs[0].args.amount);
    assert.equal(
      userHatRewards.toString(),
      (await hatToken.balanceOf(staker)).toString()
    );
  ...
  it("withdraw+ deposit + addition HAT ", async () => {
    ...
    assert.equal(
      (await hatToken.balanceOf(staker)).sub(logs[0].args.amount).toString(),
      web3.utils.toWei("1")
    );
    tx = await newVault.redeem(web3.utils.toWei("2"), staker2, staker2, { from: staker2 });
    logs = await rewardController.getPastEvents('SafeTransferReward', {
        fromBlock: tx.blockNumber,
        toBlock: tx.blockNumber
    });
