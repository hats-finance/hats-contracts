...
const HATVaultsTokenWrapper = artifacts.require("./HATVaultsTokenWrapper.sol");
...
contract("HatVaults using HatVaultsTokenWrapper", (accounts) => {
  ...
  async function calculateExpectedReward(staker, operationBlocksIncrement = 0) {
    ...
    let lpSupply = await stakingToken.balanceOf(wrappedStakingToken.address);
  ...
  //
  //     var staker = accounts[1];
  //     await stakingToken.approve(wrappedStakingToken.address,web3.utils.toWei("2"),{from:staker});
  //     await stakingToken.mint(staker,web3.utils.toWei("2"));
