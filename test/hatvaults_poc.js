const EvilMock = artifacts.require("./EvilMock.sol");
...
contract("HatVaults", (accounts) => {
  ...
  it("anyone can claim", async () => {
    ...
    assert.equal(
      (await hatToken.balanceOf(eva)).toString(),
      web3.utils.toWei("0").toString()
    );
    let evilMock = await EvilMock.new();
    await rewardController.updateVault(evilMock.address);
    await rewardController.setAllocPoint(
      evilMock.address,
      100
    );
    await rewardController.claimReward(evilMock.address, eva, { from: eva });
    assert.equal(
      (await hatToken.balanceOf(eva)).toString(),
      "259600000000000000000"
    );
  });
