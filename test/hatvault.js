...
contract('HatVault',  accounts =>  {
    ...
    it("stake", async () => {
        ...
        await setup(accounts);
        try {
            await hatVault.stake(1000,{from:staker});
      ...
      //  assert.equal(await hatVault.lpBalances(staker), 1000);
        //withdraw
        await hatVault.withdraw(1000,{from:staker});
        //lptoken burned
    ...
    it("enable farming  + getReward", async () => {
      ...
      //exit
        ...
        await hatVault.exit({from:staker});
  ...
  it("approve+ stake + exit", async () => {
    ...
    await setup(accounts);
    await stakingToken.approve(hatVault.address,web3.utils.toWei("1"),{from:staker});
    await stakingToken.approve(hatVault.address,web3.utils.toWei("1"),{from:staker2});
    await stakingToken.mint(staker,web3.utils.toWei("1"));
  ...
  //exit
    ...
    await hatVault.stake(web3.utils.toWei("1"),{from:staker2});
    assert.equal(await stakingToken.balanceOf(staker),0);
    await hatVault.exit({from:staker});
