...
const setup = async function (
  ...
  await hatTimelockController.addPool(allocPoint,stakingToken.address,accounts[1],rewardsLevels,rewardsSplit,"_descriptionHash",[86400,10]);
...
contract('HatVaults',  accounts =>  {
    ...
    it("addPool", async () => {
      ...
      try {
          await hatTimelockController.addPool(100,hatToken.address,accounts[1],[],[0,0,0,0,0,0],"_descriptionHash",[86400,10], {from:accounts[1]});
          assert(false, 'only governance');
        } catch (ex) {
          assertVMException(ex);
      }
      await hatTimelockController.addPool(100,hatToken.address,accounts[1],[],[0,0,0,0,0,0],"_descriptionHash",[86400,10]);
    });
    ...
    it("setPool", async () => {
      ...
      try {
          await hatTimelockController.setPool(0, 200,true,false,'_descriptionHash', {from: accounts[1]});
          assert(false, 'only governance');
        } catch (ex) {
          assertVMException(ex);
      }
      ...
      await hatTimelockController.setPool(0, 100,true, false,'_descriptionHash');
      await hatTimelockController.setPool(0, 200,true,false,'_descriptionHash');
    ...
    it("swapBurnSend", async () => {
      ...
      try {
          await hatTimelockController.swapBurnSend(0, accounts[1],0,[0,0],{from:accounts[3]});
          assert(false, 'only gov');
        } catch (ex) {
          assertVMException(ex);
      }
      ...
      var tx = await hatTimelockController.swapBurnSend(0,accounts[1],0,[0,0],{from:accounts[0]});
